from sms_alert import send_theft_alert
...
# ========== RECENT ALERTS TABLE (ADDED FEATURE) ==========
st.markdown("## 🚨 Recent Alerts")

# Create alerts data matching your screenshot
alerts_data = {
    "Time": ["10:34", "17:34"],
    "Meter ID": ["MTR-001", "MTR-001"],
    "Type": ["high_power", "high_power"],
    "Confidence": ["86%", "87%"],
...
with st.sidebar:
    ...
    if st.button("Send Test SMS Alert"):
        result = send_theft_alert("TEST-METER", "Test Alert", 95, "This is a test message")
        if result:
...
with st.sidebar:
    ...
    # Phone number input
    phone = st.text_input("Phone Number", value="+917543907912", key="alert_phone")
    
    if st.button("📱 Send Test SMS", key="test_sms_btn"):
        result = send_theft_alert("MTR-001", "Test Alert", 95, "This is a test from GridGuard")
        if result:
